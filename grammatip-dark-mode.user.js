...
(function() {
    ...
    // Smooth transition
    sheet.insertRule('* { transition: background-color 0.25s; }', sheet.cssRules.length);

    // Baggrund
    sheet.insertRule('#main-content, .header, .nav-tabs > .active > a, .modal, .lp.background, .video-container .video-section, .user-settings-content { background-color: '+defaultPalette.background+' !important }', sheet.cssRules.length);
    ...
    // Tekst
    sheet.insertRule('body, h1, h2, h3, h4, h5, h6, p, .nav-tabs > .active > a, li, input.dictationTextField { color: '+defaultPalette.text+' !important }', sheet.cssRules.length);

    // Kanter
    sheet.insertRule('.nav > li > a:hover, .student-intro-exercise-box, .student-intro-feedback, input[type="text"] { background-color: '+defaultPalette.border+' !important }', sheet.cssRules.length);
    sheet.insertRule('.nav-tabs, .student-stat .student-table td, .student-table, .nav-tabs > .active > a, .nav-tabs > li > a:hover, .student-intro-exercise-box, .student-intro-feedback, .modal-header, input.dictationTextField { border-color: '+defaultPalette.border+' !important }', sheet.cssRules.length);
